    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div style={{ borderTop: `3px solid ${ORANGE}`, paddingTop: 16 }}>
        ...
        <p style={{ fontFamily: SF, fontSize: 14, fontStyle: "italic", color: t.mid, lineHeight: 1.7, maxWidth: 640 }}>
          Politician accountability scores, STOCK Act violations, and dark money network traces — the full corruption signals layer.
        </p>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${t.border}` }}>
        {SUBTABS.map(st => (
          <button key={st.id} onClick={() => setSub(st.id)} style={{
            ...
            padding: "10px 20px", fontFamily: MF, fontSize: 10.5, letterSpacing: 0.5,
      ...
      {sub === "stockact"       && <StockActMonitor theme={t} />}
      {sub === "darkmoney"      && <DarkMoneyTracker theme={t} />}
    </div>
  );
}
