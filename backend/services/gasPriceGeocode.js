...
/**
 * Geocode a query string (city, ZIP, or address) → { lat, lng, formattedAddress, state }
 */
...
async function googleGeocode(query, apiKey) {
  try {
    ...
    console.error('[GasPriceGeocode] Google error:', err.message)
    return fallbackGeocode(query)
  }
}

// Major US city centroids
const CITY_COORDS = {
...
// ZIP prefix → approximate state center
...
function fallbackGeocode(query) {
  ...
  for (const [city, coords] of Object.entries(CITY_COORDS)) {
    if (q.includes(city)) {
      return { ...coords, formattedAddress: query, source: 'fallback-city' }
    }
  }
