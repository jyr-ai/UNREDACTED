...
/**
 * Find stations near a lat/lng coordinate.
 */
export async function getStationsByLocation({ lat, lng, radiusMiles = 10, fuelType = 'regular', sortBy = 'distance' }) {
 ...
 */
export async function getStationsByZip({ zip, radiusMiles = 10, fuelType = 'regular', sortBy = 'distance' }) {
  const apiKey   = process.env.MYGASFEED_API_KEY
  const fuelCode = FUEL_TYPES[fuelType] || 'reg'
...
/** Normalize a raw MyGasFeed station into a consistent shape. */
